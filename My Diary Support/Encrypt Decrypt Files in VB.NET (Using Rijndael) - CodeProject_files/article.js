...
/*
	This code wraps PRE blocks in collapse / copy code commands
*/
...
var processCodeBlocks = new function () {
    ...
    var _self                = this;
    var _minHeightToCollapse = 200;
    var _collapseCodeOnLoad  = false;
    ...
    this.Initialise = function (parentSelector, collapseHeight, collapseCodeOnLoad) {

        _minHeightToCollapse = collapseHeight || _minHeightToCollapse;
        _collapseCodeOnLoad  = !!collapseCodeOnLoad;
        ...
        $(parentSelector + " pre").each(function (preIndex) {
            var $preBlock = $(this);

            if ($preBlock.decoration && $preBlock.decoration.toLowerCase() == 'none')
                return;

            $preBlock.attr("id", "pre" + preIndex.toString());
            $preBlock.css("margin-top", "0");

            var html = "<div class='pre-action-link' id='premain" + preIndex.toString() + "' "
                     + " style='width:100%;display:block;'>";

            // Invisible / display:none elements have height = 0
            if ($preBlock.height() <= 0 || $preBlock.height() > _minHeightToCollapse) {

                var initialImage = '/images/arrow-up-16.png';
                var initialText  = 'Collapse';

                if (_collapseCodeOnLoad && $preBlock.height() > 0) {
                    initialImage = '/images/arrow-down-16.png';
                    initialText  = 'Expand';
                }

                html += "<span id='precollapse" + preIndex.toString() + "' "
                     + " style='cursor:pointer;margin:0;' "
                     + " onclick='processCodeBlocks.togglePre(" + preIndex.toString() + ");'>"
                     + initialText + "</span> "
                     + "<img id='preimg" + preIndex.toString() + "' src='" + initialImage + "' "
                ...
                if (_collapseCodeOnLoad && $preBlock.height() > 0) {
                    ...
                    $preBlock.height(_minHeightToCollapse);
                    $preBlock.attr('collapse', 'true');
                }
            }

            html += "<a href='#' onclick='return processCodeBlocks.copyCode(" + preIndex.toString() + ");'>"
                 + "Copy Code</a>"
                 + "</div>";
    ...
    this.togglePre = function (id) {
        ...
        var $togelm = $("#precollapse" + id);

        // $preelm.animate({ opacity: "toggle", height: "toggle" }, "fast");

        if ($preelm.attr('collapse') === 'true') {
            $imgelm.attr("src", "/images/arrow-up-16.png");
            $togelm.text(" Collapse");
            $preelm.attr('collapse', 'false');
            // $preelm.animate({ height: "auto" }, "fast");
            $preelm.height('auto');
        } else {
            $imgelm.attr("src", "/images/arrow-down-16.png");
            $togelm.text(" Expand");
            $preelm.attr('collapse', 'true');
            $preelm.animate({ height: Math.min(_minHeightToCollapse,  $preelm.height()) }, "fast");
        }
    }
